...
describe("learning scenario catalog", () => {
  ...
  it("marks only implemented scenarios as available", () => {
    expect(scenarioCatalog.filter((scenario) => scenario.status === "available").map((scenario) => scenario.id)).toEqual(["retry-storm", "cache-stampede", "queue-consumer-lag", "connection-pool-exhaustion"]);
    expect(scenarioCatalog.filter((scenario) => scenario.status === "planned")).toHaveLength(6);
  });
});
